(function(){
  ...
  function exLabel(){
    var ex=state.exceptionChoice || (typeof selectedException==='function'?selectedException():null) || {};
    ...
    if(ex.id==='repair') return ex.title || ex.repairType || '보정, 수리(제26조제1항제15호)';
    return ex.title || '-';
  }
  function isConsumerUnreleased(){
    var ex=state.exceptionChoice || {};
    var extra=state.exceptionExtra || {};
    return ex.id==='consumer' && (ex.consumerReleased==='no' || extra.consumerReleased==='no');
  }
  function firstModel(){
  ...
  function buildDoc(){
    ...
    var body='<p>'+(countriesText()||'{국가명}')+' 상황허가 대상품목 수출을 위한 사전신고 품의를 다음과 같이 상신합니다.</p><h4>1. 사전신고 신청건</h4>'+tableHtml(rows)+'<h4>2. 첨부파일</h4>'+attachmentsHtml();
