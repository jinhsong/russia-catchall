(function(){
  ...
  function drawExtra(){
    ...
    if(id==='repair'){
      ...
      anchor.insertAdjacentHTML('beforebegin','<div id="ex-extra-detail" class="note info"><b>세부 허가예외를 선택하세요.</b><div style="margin-top:8px"><label class="radio"><input type="radio" name="ex-repair-type" value="보정, 수리(제26조제1항제15호)" '+(saved==='보정, 수리(제26조제1항제15호)'?'checked':'')+'> <span><b>보정, 수리</b><br><span class="muted small">제26조제1항제15호</span></span></label><label class="radio"><input type="radio" name="ex-repair-type" value="검사, 시험(제26조제1항제16호)" '+(saved==='검사, 시험(제26조제1항제16호)'?'checked':'')+'> <span><b>검사, 시험</b><br><span class="muted small">제26조제1항제16호</span></span></label></div></div>');
      document.querySelectorAll('input[name="ex-repair-type"]').forEach(function(r){r.onchange=function(){extra().repairType=this.value;document.querySelectorAll('input[name="ex-repair-type"]').forEach(function(x){x.closest('.radio').classList.toggle('on',x.checked);});save();};r.closest('.radio').classList.toggle('on',r.checked);});
    }
    if(id==='consumer'){
      var saved2=extra().consumerReleased||'';
      anchor.insertAdjacentHTML('beforebegin','<div id="ex-extra-detail" class="note info"><b>이미 출시된 제품입니까?</b><div style="margin-top:8px"><label class="radio"><input type="radio" name="ex-consumer-released" value="yes" '+(saved2==='yes'?'checked':'')+'> <span><b>예</b><br><span class="muted small">사전신고 시 삼성닷컴 등 오픈마켓에서 판매 중인 화면 캡처를 첨부하세요.</span></span></label><label class="radio"><input type="radio" name="ex-consumer-released" value="no" '+(saved2==='no'?'checked':'')+'> <span><b>아니오</b><br><span class="muted small">6개월 이내 출시 확약서가 필요합니다.</span></span></label></div><div id="ex-consumer-guide" style="margin-top:8px"></div></div>');
      var guide=function(){var v=document.querySelector('input[name="ex-consumer-released"]:checked')?.value||'',g=document.getElementById('ex-consumer-guide'); if(!g)return; if(v==='yes')g.innerHTML=note('ok','사전신고 시 <b>삼성닷컴 등 오픈마켓에서 판매 중인 화면 캡처</b>를 첨부해 주세요.'); if(v==='no')g.innerHTML=note('warn','<b>6개월 이내 출시 확약서(산업통상자원부 제출)</b>가 필요합니다.<br><a class="btn sm" href="docs/6month_launch_commitment.docx" download>6개월 이내 출시 확약서 다운로드</a><ul><li>본 6개월 이내 출시 확약서에 서명 후 파트장 결재, 통상그룹 통보로 진행해 주세요.</li><li>사전신고 시 본 6개월 이내 출시 확약서와 결재본(eml 파일)을 첨부해야 합니다.</li></ul>');};
      document.querySelectorAll('input[name="ex-consumer-released"]').forEach(function(r){r.onchange=function(){extra().consumerReleased=this.value;document.querySelectorAll('input[name="ex-consumer-released"]').forEach(function(x){x.closest('.radio').classList.toggle('on',x.checked);});guide();save();};r.closest('.radio').classList.toggle('on',r.checked);}); guide();
    }
  }
